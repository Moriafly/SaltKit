@file:Suppress("UNUSED")

/**
 * SaltKit
 * Copyright (C) 2023 Moriafly
 *
 * This library is free software; you can redistribute it and/or
 * modify it under the terms of the GNU Lesser General Public
 * License as published by the Free Software Foundation; either
 * version 2.1 of the License, or (at your option) any later version.
 *
 * This library is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU
 * Lesser General Public License for more details.
 */

package com.moriafly.salt.kit.sort

import com.moriafly.salt.kit.UnstableSaltKitApi

object SortKit {

    /**
     * Sort chars by natural, implemented sorting of numbers in strings
     *
     * @param comparator Custom not digit
     */
    @UnstableSaltKitApi
    inline fun <T> sortedByNatural(
        iterable: Iterable<T>,
        comparator: AbstractNaturalComparator = NaturalComparator,
        crossinline selector: (T) -> CharSequence
    ): List<T> {
        return iterable.sortedWith { a, b ->
            comparator.compare(selector(a), selector(b))
        }
    }

}